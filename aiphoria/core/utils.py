...
# Global output directory state
_global_output_directory: Optional[str] = None


def get_output_directory() -> Optional[str]:
    """
    Get the currently set global output directory.
    
    :return: Absolute path to output directory, or None if not set
    """
    global _global_output_directory
    return _global_output_directory


def set_output_directory(output_dir_path: str) -> str:
    """
    Set the global output directory for the package.
    This is called by create_output_directory() and can be called directly.
    
    :param output_dir_path: Absolute path to output directory
    :return: Absolute path to the output directory
    """
    global _global_output_directory
    _global_output_directory = os.path.abspath(output_dir_path)
    return _global_output_directory
...
def create_output_directory(output_dir_path: str) -> str:
    """
    Create and set the global output directory for the package.
    
    This function:
    1. Creates the directory (deletes if exists)
    2. Sets it as the global output directory for all package output operations
    
    All output files generated by the package will be saved to this directory.
    
    :param output_dir_path: Absolute or relative path to output directory
    :return: Absolute path to the created output directory
    """
    # Convert to absolute path
    abs_output_path = os.path.abspath(output_dir_path)
    ...
    set_output_directory(abs_output_path)
...
def setup_scenario_output_directories(output_dir_path: str, scenario_names: List[str]) -> Dict[str, str]:
    ...
    This function also sets the main output directory as the global output directory.

    :param output_dir_path: Output directory name
    :param scenario_names: List of scenario names
    :return: Dictionary [str, str]: Scenario name to scenario directory (absolute path)
    """
    # First, set the global output directory
    abs_output_dir = os.path.abspath(output_dir_path)
    set_output_directory(abs_output_dir)
    
    scenario_name_to_abs_scenario_output_path = {}
    for scenario_name in scenario_names:
        abs_scenario_output_path = os.path.join(abs_output_dir, scenario_name)
        try:
...
def get_output_file_path(filename: str, scenario_name: Optional[str] = None) -> str:
    """
    Get the full path for an output file, respecting the global output directory.
    
    This function helps ensure all output files are saved to the configured output directory.
    
    :param filename: Name of the file (e.g., 'results.csv')
    :param scenario_name: Optional scenario name for scenario-specific outputs
    :return: Absolute path to the output file
    :raises RuntimeError: If output directory has not been set
    """
    ...
    if global_output_dir is None:
        raise RuntimeError(
            ...
            "Call create_output_directory() or setup_scenario_output_directories() first."
        )
    
    if scenario_name:
        return os.path.join(global_output_dir, scenario_name, filename)
    else:
        return os.path.join(global_output_dir, filename)
